...
RegisterNUICallback('craft', function(data)
    ...
    QBCore.Functions.TriggerCallback('qw-crafting:server:enoughMaterials', function(hasMaterials)
        if hasMaterials then
            ...
            QBCore.Functions.Progressbar('crafting-prog', 'Chế tạo...', 5000, false, false, {
                ...
                if Config.UsingSkills then 
                    ...
                    if base > random then
                        exports["mz-skills"]:UpdateSkill("Chế tạo", 1)
                    end
                end
            end, function() -- Cancel
                ClearPedTasks(PlayerPedId())
                QBCore.Functions.Notify('Bạn đã hủy quá trình chế tạo', 'error')
            end)

        else
            QBCore.Functions.Notify("Bạn không có đủ nguyên liệu để chế tạo vật phẩm này", "error")
            TriggerEvent('animations:client:EmoteCommandStart', { "c" })
        end
...
function createCraftingZones()
    ...
    for k, v in pairs(Config.CraftingLocations) do
        if v.job ~= nil then
            exports['qb-target']:AddCircleZone(k .. "-Chế tạo", v.location, 1.0, {
                name = k .. "--Chế tạo",
...
RegisterNetEvent('qw-crafting:client:openCrafting', function(tableName)
    TriggerEvent('animations:client:EmoteCommandStart', { "doxe" })
...
AddEventHandler('onResourceStop', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        for k, v in pairs(Config.CraftingLocations) do
            exports['qb-target']:RemoveZone(k .. '-Chế tạo')
        end
    end
end)
